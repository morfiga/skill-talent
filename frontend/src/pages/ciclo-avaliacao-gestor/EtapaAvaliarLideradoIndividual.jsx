...
function EtapaAvaliarLideradoIndividual({ colaboradorId, cicloAberto, lideradoSendoAvaliado, onFinalizado, onVoltar }) {
  ...
  useEffect(() => {
    if (lideradoSendoAvaliado) {
      loadEixosAvaliacao()
    }
  }, [lideradoSendoAvaliado])

  useEffect(() => {
    if (lideradoSendoAvaliado && cicloAberto && colaboradorId) {
      loadAvaliacaoExistente()
    }
  }, [lideradoSendoAvaliado, cicloAberto, colaboradorId])
  ...
  const handleSalvar = async () => {
    if (isAvaliacaoCompleta() && lideradoSendoAvaliado && cicloAberto) {
      try {
        setSalvando(true)
        const eixos = {}
        ...
        try {
          ...
          if (avaliacoes.length > 0) {
            ...
            success(`Avaliação de ${lideradoSendoAvaliado.nome} atualizada com sucesso!`)
          } else {
            ...
            success(`Avaliação de ${lideradoSendoAvaliado.nome} salva com sucesso!`)
          }
        } catch (createError) {
        ...
        handleApiError(error, 'salvar avaliação do liderado', '/avaliacoes', showError)
      } finally {
        setSalvando(false)
      }
  ...
  return (
    <>
      <div className="ciclo-header">
        <h2 className="ciclo-step-title">Avaliar {lideradoSendoAvaliado.nome}</h2>
        <p className="ciclo-step-description">
          Avalie o desempenho de {lideradoSendoAvaliado.nome} em cada eixo e forneça feedback construtivo
        </p>
      </div>
      ...
      <div className="autoavaliacao-section">
            ...
            <div key={eixo.id} className="eixo-card">
              ...
              <div className="niveis-container">
                <p className="niveis-label">
                  Selecione o nível de {lideradoSendoAvaliado.nome}:
                </p>
              ...
              <div className="justificativa-container">
                ...
                <textarea
                  ...
                  placeholder={`Compartilhe seu feedback sobre ${lideradoSendoAvaliado.nome} neste eixo...`}
        ...
        <div className="avaliacao-geral-container">
          <h3 className="avaliacao-geral-title">
            Feedback Geral sobre {lideradoSendoAvaliado.nome}
          </h3>
          <label className="avaliacao-geral-label">
            Compartilhe seu feedback geral sobre {lideradoSendoAvaliado.nome} <span className="required">*</span>
          </label>
          <textarea
            className="avaliacao-geral-textarea"
            placeholder={`Descreva suas percepções gerais sobre ${lideradoSendoAvaliado.nome}, pontos fortes, oportunidades de desenvolvimento e recomendações...`}
...
export default EtapaAvaliarLideradoIndividual
