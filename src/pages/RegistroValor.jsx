  ...
  const handleSalvarRegistro = () => {
    if (
      ...
      const valoresNomes = formulario.valoresSelecionados.map(id => {
        const valor = valoresDisponiveis.find(v => v.id === id)
        return valor ? valor.nome : id
      })

      const novoRegistro = {
        id: Date.now(),
        descricao: formulario.descricao,
        valores: valoresNomes,
        valoresIds: formulario.valoresSelecionados,
              ...
              <div key={registro.id} className="registro-card">
                <div className="registro-card-header">
                  <div className="registro-data">
                    ...
                    <span className="data-text">{registro.data}</span>
                  </div>
                  <div className="registro-valores">
                    {registro.valores.map((valorNome, index) => {
                      const valor = valoresDisponiveis.find(v => v.nome === valorNome)
                      return (
                        <span key={index} className="valor-badge">
                          {valor?.icone} {valorNome}
                        </span>
                      )
                    })}
                  </div>
