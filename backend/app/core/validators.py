...
NIVEIS_CARREIRA_VALIDOS = ["E", "J1", "J2", "J3", "P1", "P2", "P3", "S1", "S2", "S3", "ES1", "ES2"]
...
def validate_pares_ids(pares_ids: List[int]) -> List[int]:
    ...
    if len(pares_ids) != NUMERO_PARES_OBRIGATORIO:
        raise ValueError(f"É necessário selecionar exatamente {NUMERO_PARES_OBRIGATORIO} pares")
...
def validate_texto_obrigatorio(texto: str, campo: str, max_length: int = CAMPO_TEXTO_MAX) -> str:
    """
...
def validate_texto_opcional(texto: Optional[str], campo: str, max_length: int = CAMPO_TEXTO_MAX) -> Optional[str]:
    """
